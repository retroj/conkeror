...
/**
 * Function for creating custom Wikipedia webjumps. The arguments being passed
 * are the prefixes for the different Wikipedias. "en" means "English" (because
 * the address is "http://en.wikipedia.org"), etc.
 *
 * If you wish to define webjumps for all of the webjumps, simply pass "ALL" as
 * the only argument. Any argument after "ALL" will be ignored.
 *
 * Examples:
 ...
 * define_wikipedia_webjumps("ALL"); // All of the available Wikipedias.
 */
function define_wikipedia_webjumps() {

    // Erroneous input.
    if (arguments.length == 0) {
	dumpln("`define_wikipedia_webjumps' takes at least one argument.");
	return;
    }

    // If the first argument is "ALL", define all of the webjumps.
    if (arguments[0] == "ALL") {
	for (prefix in wikijumps) {
	    let rest = wikijumps[prefix];
	    define_webjump(prefix,
			   "http://" + prefix + ".wikipedia.org/wiki/" + rest);
	}
	return;
    }
	...
	prefix = arguments[i];
	rest = wikijumps[prefix];
	define_webjump(prefix,
		       "http://" + prefix + ".wikipedia.org/wiki/" + rest);
    }
}

var wikijumps = { "aa"	     : "Special:Search?search=%s&go=Go",
